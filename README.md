...
- Vitest + ESLint
- 可配置翻译 provider：默认 MyMemory 免费在线翻译，也支持百度、网易有道、微软、Google public endpoint 和本地 mock；AI 分析部分目前是本地 mock 逻辑，接口已封装，后续可替换为 OpenAI 或其他 provider。
...
默认使用 MyMemory 免费在线翻译；也可以在 `Settings` 页面切换翻译引擎。服务端也支持用环境变量设置初始值：

```bash
MAGREADER_TRANSLATION_PROVIDER=mymemory npm run dev
```
...
百度、网易有道、微软都需要官方开发者密钥；未配置密钥时，Settings 中可以选择该引擎，但翻译请求会返回明确的配置错误。

如需使用百度翻译开放平台：

```bash
MAGREADER_TRANSLATION_PROVIDER=baidu \
BAIDU_TRANSLATE_APP_ID=your_app_id \
BAIDU_TRANSLATE_SECRET_KEY=your_secret_key \
npm run dev
```

如需使用 Google Translate public endpoint：

```bash
MAGREADER_TRANSLATION_PROVIDER=google npm run dev
```

如需使用网易有道智云文本翻译，需要先在有道智云控制台创建文本翻译应用，然后配置：

```bash
MAGREADER_TRANSLATION_PROVIDER=netease \
YOUDAO_APP_KEY=your_app_key \
YOUDAO_APP_SECRET=your_app_secret \
npm run dev
```

如需使用 Microsoft Translator：

```bash
MAGREADER_TRANSLATION_PROVIDER=microsoft \
MICROSOFT_TRANSLATOR_KEY=your_key \
MICROSOFT_TRANSLATOR_REGION=your_region \
npm run dev
```

如果你的 Azure Translator 使用自定义 endpoint，还可以设置：
...
```bash
MAGREADER_TRANSLATION_PROVIDER=mock npm run dev
```
...
- 当前没有登录、云同步或多用户权限系统。
- 翻译请求会把选中文本发送到当前配置的翻译 provider；mock AI 分析在本地完成。
...
翻译依赖网络访问当前配置的翻译 provider。若配置为有道，需要确认 `YOUDAO_APP_KEY` 和 `YOUDAO_APP_SECRET` 已正确设置；若网络不可用或请求被限制，Learning Panel 会显示失败状态。可以稍后重试。
